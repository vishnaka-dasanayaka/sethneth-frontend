...
@Component({
  selector: "app-add-inovice-item",
  standalone: false,
  templateUrl: "./add-inovice-item.component.html",
  styleUrl: "./add-inovice-item.component.css",
})
export class AddInoviceItemComponent {
  ...
  constructor(
    ...
    this.valForm = this.fb.group({
      ...
      model: [null],
      lense: [null],
      unit_price: [{ value: null, disabled: true }, Validators.required],
      qty: [1, Validators.required],
      total_price: [{ value: 0, disabled: true }, Validators.required],
      discount: [0, Validators.required],
      discounted_price: [{ value: 0, disabled: true }, Validators.required],
    });

    this.clickEventSubscription = this.sharedService
      .getAddInvoiceItemClickEvent()
      .subscribe(() => {
        this.openModal();
  ...
  submitForm(value: any) {
    ...
    if (this.valForm.valid) {
      ...
      var data = this.sharedService.getInvoiceItemData();
      value.inv_id = data.inv_id;

      if (value.category !== "lense") {
      ...
      this.invoiceService.createInvItem(value).subscribe(
        (data) => {
          if (data.status) {
            this.parentFun.emit();
            this.closeModal();
  ...
  updatePrice() {
    var qty = this.valForm.get("qty")?.value;
    var discount = this.valForm.get("discount")?.value;
    var unit_price = this.valForm.get("unit_price")?.value;

    this.valForm.patchValue({ total_price: unit_price * qty });
    this.valForm.patchValue({ discounted_price: unit_price * qty - discount });
  }
